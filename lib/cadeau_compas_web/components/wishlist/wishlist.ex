defmodule CadeauCompasWeb.Components.WishlistList do
    ...
    <.card class="max-w-4xl mx auto">
      <%= if @editable do %>
        <.accordion_trigger open={@wishlist.products != [] && @index == 0} class="flex flex-row-reverse p-2">
          <.wishlist_header wishlist={@wishlist} editable={@editable}></.wishlist_header>
        </.accordion_trigger>
        <.accordion_content>
          <.wishlist_content wishlist={@wishlist}></.wishlist_content>
        </.accordion_content>
      <% else %>
        <.wishlist_header wishlist={@wishlist} editable={@editable}></.wishlist_header>
        <.wishlist_content wishlist={@wishlist}></.wishlist_content>
