defmodule CadeauCompasWeb.Components.SearchModal do
  ...
  def search_modal(assigns) do
    ...
    <div id="searchbar-dialog" class="hidden fixed inset-0 z-50" role="dialog" aria-modal="true" phx-window-keydown={hide_search_modal()} phx-key="escape">
      ...
      <div class="fixed inset-0 overflow-y-auto px-4 py-4 sm:py-20 sm:px-6 md:py-32 lg:px-8 lg:py-[15vh]">
        <div id="searchbox_container" class="mx-auto overflow-hidden rounded-lg bg-zinc-50 shadow-xl ring-zinc-900/7.5 sm:max-w-xl opacity-100 scale-100" phx-hook="SearchModal">
          <div role="combobox" aria-haspopup="listbox" phx-click-away={hide_search_modal()} aria-expanded={@search_entries != []}>
            <form action="" novalidate="" role="search" phx-change="update_search_query">
              ...
              <ul :if={@search_entries != []} class="divide-y divide-slate-200 overflow-y-auto rounded-b-lg border-t border-slate-200 text-sm leading-6" id="searchbox__results_list" role="listbox">
                <%= for entry <- @search_entries do %>
                  <%= if @entry_slot != [] do %>
                    <%= render_slot(@entry_slot, entry) %>
                  <% else %>
                    <.link phx-click="click_search_entry" phx-value-id={entry.id} id={"#{entry.id}"} class="block p-4 hover:bg-slate-100 focus:outline-none focus:bg-slate-100 focus:text-sky-800">
                      <%= entry.name %>
                    </.link>
                  <% end %>
                <% end %>
  ...
  def hide_search_modal(js \\ %JS{}) do
    ...
    |> JS.hide(
      ...
      transition: {"transition ease-in duration-100", "opacity-100 scale-100", "opacity-0 scale-95"}
    )
    |> JS.hide(
      to: "#searchbar-dialog",
